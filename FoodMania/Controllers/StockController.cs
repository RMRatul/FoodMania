        ...
        public ActionResult StockItem(CRU_StockItemMV cru_StockItemMV)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                else
                {
                    ...
                    if (checkexist == null)
                    {
                        var eidtitem = Db.StockItemTables.Find(cru_StockItemMV.StockItemID);
                        eidtitem.StockItemCategoryID = cru_StockItemMV.StockItemCategoryID;
                        eidtitem.StockItemTitle = cru_StockItemMV.StockItemTitle;
                        eidtitem.ItemSize = cru_StockItemMV.ItemSize;
                        eidtitem.UnitPrice = cru_StockItemMV.UnitPrice;
                        eidtitem.VisibleStatusID = cru_StockItemMV.VisibleStatusID;
                        eidtitem.OrderTypeID = cru_StockItemMV.OrderTypeID;
                        Db.Entry(eidtitem).State = System.Data.Entity.EntityState.Modified;
                        Db.SaveChanges();

                        if (cru_StockItemMV.PhotoPath != null)
                        {
                            var folder = "~/Content/StockItemPhoto";
                            var photoname = string.Format("{0}.jpg", eidtitem.StockItemID);
                            var response = HelperClass.FileUpload.UploadPhoto(cru_StockItemMV.PhotoPath, folder, photoname);
                            if (response)
                            {
                                var photo = string.Format("{0}/{1}", folder, photoname);
            ...
            if (ModelState.IsValid)
            {
                ...
                else
                {
                    var checkexist = Db.StockDealDetailTables.Where(
                       s => s.StockItemID == cru_StockDealDetailMV.StockItemID
                       && s.VisibleStatusID == 1
                       && s.StockDealDetailsID != cru_StockDealDetailMV.StockDealDetailID
                       && s.StockDealID == cru_StockDealDetailMV.StockItemID).FirstOrDefault();
                    if (checkexist == null)
                    {
